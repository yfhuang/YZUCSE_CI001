# Supplement 04: Learning Input Patterns via UVa Problems
...
| Token header plus full lines | Read a number first, then switch to `getline` | UVa 10008, 10142, 10188, 482 | `cin.ignore(...); getline(...)` |
...
The input begins with one or more numbers, then the real content is stored line by line. These problems usually require switching from token reading to line reading after the header.
...
Use `getline`, skip blank lines before each block, and collect lines until the next blank line.
...
- Skipping leading blank lines makes the pattern more robust when the separator is not exactly one line.
